...
selected, everything else (which runs are in the queue, the rubric, existing
feedback, marking a run complete) is fetched via `window.langsmith.call`. See
`src/api.ts` and `src/components/QueueBar.tsx`.
