...
experiment-comparison`. Pick a dataset, a baseline experiment, and one or more
comparisons; it shows per-experiment aggregates and a per-example table, with
comparison cells colored green/red vs the baseline — for latency, cost, and
tokens too, not just feedback scores. **Read `AGENTS.md` first** — it documents
the LangSmith API surface this app can call.
...
and the preview reloads on its own. Pick a dataset and experiments from the in-app dropdowns
once they load (it calls the real API using your local `LANGSMITH_API_KEY`).
