...
func TestDatasetTraceBlankSelectors(t *testing.T) {
	for _, name := range []string{"trace-id", "run-ids", "trace-ids"} {
		cmd := newDatasetPreviewTracesCmd()
	...
	cmd := newDatasetAddTracesCmd()
	cmd.SetArgs([]string{"--dataset", importDatasetID, "--project-id", deleteTestProjectID, "--trace-id", ""})
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
...
func TestDatasetTraceImportRetryAndFailures(t *testing.T) {
	for _, mode := range []string{"success", "direct", "ambiguous", "denied", "conflict", "wrong-source"} {
		t.Run(mode, func(t *testing.T) {
			...
			run := func() (string, error) {
				...
				cmd.SetArgs([]string{"--selection", path, "--dataset", importDatasetID, "--project-id", deleteTestProjectID})
				if mode == "direct" {
					cmd.SetArgs([]string{"--trace-id", importRunID, "--dataset", importDatasetID, "--project-id", deleteTestProjectID})
				}
				cmd.SetErr(io.Discard)
				cmd.SetOut(io.Discard)
				var err error
			...
			out, err := run()
			if mode == "success" || mode == "direct" || mode == "ambiguous" {
				if err != nil || writes != 1 {
