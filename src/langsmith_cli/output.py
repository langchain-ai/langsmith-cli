...
from langsmith_cli.utils import calc_duration, format_duration
...
def output_json(data, file_path: str | None = None) -> None:
    ...
    if file_path:
        with open(file_path, "w") as f:
            f.write(json_str)
        click.echo(f'{{"status": "written", "path": "{file_path}"}}', err=True)
    else:
        click.echo(json_str)
...
def output_jsonl(items: list, file_path: str | None = None) -> None:
    ...
    if file_path:
        ...
        click.echo(f'{{"status": "written", "path": "{file_path}", "count": {len(items)}}}', err=True)
    else:
        for item in items:
